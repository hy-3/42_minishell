...
int	fill_till_quote_closed(char *original_str, t_fill *fill, t_parse_param *parse_p, int k)
{
	char	tmp_quote;
...
void	fill_str(char *original_str, t_fill *fill, t_parse_param *parse_p, t_env_param *env_p)
{
	...
	while (original_str[fill->start] != ' ' && original_str[fill->start] != '\0' && original_str[fill->start] != '>' && original_str[fill->start] != '<')
	{
		...
		{
			k = fill_till_quote_closed(original_str, fill, parse_p, k);
			if (k == -1)
				return ;
			continue ;
		}
	...
	if (is_dollar_exist(fill->tmp_str) == 1 && (fill->current_quote == 0 || fill->current_quote == 34))
		fill->list->str = convert_str_from_dollar(fill->tmp_str, fill->current_quote, fill->end_pos_of_dollar_var, env_p);
	else
		fill->list->str = ft_strdup(fill->tmp_str);
}
